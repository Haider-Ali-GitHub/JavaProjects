...
public class ForgeStormBreaker {
	public static void main (String [] args) {
        ...
        int totalFlux = 0; 
        int stormBreakerArray[][] = new int[rows][columns];
    
        for(int row = 0; row < stormBreakerArray.length; row++)
        {for(int col = 0; col < stormBreakerArray[row].length; col++)
         {
            int flux = StdIn.readInt(); 
            stormBreakerArray[row][col] = flux;   
            totalFlux += flux; // WRITE YOUR CODE HERE TO CALCULATE THE FLUX USING THE INPUT FILE
         }
        }  
        StdOut.setFile(forgeStormbreakerOutputFile); // Set the output file.
        StdOut.print(totalFlux);  // WRITE YOUR CODE HERE TO OUTPUT TO THE OUTPUT FILE
    }
}
