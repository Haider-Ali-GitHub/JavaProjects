...
public class MindStoneNeighborNeurons {
    public static void main (String [] args) {       
        ...
        int nueronCount = StdIn.readInt();
        String totalNuerons[]= new String[nueronCount];

        for(int i = 0 ; i < totalNuerons.length; i++)
        {
            totalNuerons[i] = StdIn.readString(); 
        }

        int synapsesCount = StdIn.readInt();
        String totalSynapses[][]= new String[synapsesCount][2];

        for(int i = 0 ; i < totalSynapses.length; i++)
        {
            totalSynapses[i][0] = StdIn.readString();
            totalSynapses[i][1] = StdIn.readString();
        }

        String Node = null;
        for(int i = 0 ; i < totalSynapses.length; i++)
        { for(int index = 0; index < totalNuerons.length; index++)
            {
               if(totalNuerons[index] != totalSynapses[i][0]) //tests which value doesnt show in first col 
               {
                 Node = totalNuerons[index]; //sets value to Node
               } 
            }
         }
         ...
         for(int i = 0 ; i < totalSynapses.length; i++)
         {
            //TESTER StdOut.println("index"+ i + " " + Node + " " + totalSynapses[i][1]);
            if(Node.equals(totalSynapses[i][1])) 
            {    
                StdOut.println(totalSynapses[i][0]); //prints values that point to node
            }
